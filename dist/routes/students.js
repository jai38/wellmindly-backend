 ...
 */
router.get('/me/daily-checkins', jwt_1.authenticateJWT, (0, jwt_1.authorizeRoles)('STUDENT', 'ADMIN'), async (req, res) => {
    try {
        const email = req.user?.email;
        if (!email) {
            res.status(401).json({ error: 'Email missing from token' });
...
router.post('/me/daily-checkin', jwt_1.authenticateJWT, (0, jwt_1.authorizeRoles)('STUDENT', 'ADMIN'), async (req, res) => {
    try {
        ...
        const existingCheckin = await prisma_1.default.dailyCheckin.findFirst({
            where: {
                userId: user.id,
                createdAt: {
                    gte: todayStart,
        ...
        if (existingCheckin) {
            await prisma_1.default.dailyCheckin.update({
                where: { id: existingCheckin.id },
                data: { rating },
            });
        }
        else {
            await prisma_1.default.dailyCheckin.create({
                data: {
                    userId: user.id,
                    rating,
                },
 ...
 */
router.get('/me/results', jwt_1.authenticateJWT, (0, jwt_1.authorizeRoles)('STUDENT', 'ADMIN'), async (req, res) => {
    try {
